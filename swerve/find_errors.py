...
def find_errors(data, logger=None, spike_filt_type='both'):
    ...
    if spike_filt_type == 'difference' or spike_filt_type == 'both':
        _diff_spike_filt(data_series, spike_threshold, errors)
    if spike_filt_type == 'median' or spike_filt_type == 'both':
        median_window = gic_filter_kwargs['median_window']
        _median_spike_filt(data_series, spike_threshold, errors, win=median_window)
    ...
    std_pre = float(np.std(data_df['data'][pre_mask].to_numpy()))
    std_post = float(np.std(data_df['data'][post_mask].to_numpy()))
...
def _diff_spike_filt(data, spike_threshold, errors, diff_window=5, std_window=20, dd_thresh=10, std_thresh=1):
    ...
    if spike_mask.any():
        errors.append(f"Unphysical spike detected at time {series.index[spike_mask][0]}")
